...
const ContragentView: FC<IClassNameProps> = () => {
  ...
  const loadData = () => {
    ...
    console.log('🔍 ContragentView loadData, sessionState:', sessionState);

    if (sessionState) {
      const {apiHandler, row} = JSON.parse(sessionState) as OnRowDblClick & {row: IGridRow};
      console.log('✅ ContragentView has sessionState. apiHandler, row:', apiHandler, row);
  ...
  useEffect(() => {
    console.log('🏗️ ContragentView mounted, calling loadData');
    loadData();
  },[]);

  return (
    <Layout>
      <ContragentViewStateContext.Provider value={{state, dispatch}}>
        <div className="ContragentView">
          <div className="HeaderAndTime">
            <h1>Просмотр контрагента #{state.pageHeader.id.val}</h1>
            <p className="CurrentTime">Время открытия страницы: {state.currentTime}</p>
          </div>
          <PageHeader/>
        </div>
      </ContragentViewStateContext.Provider>
    </Layout>
  );
};
